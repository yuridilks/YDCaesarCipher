...
def instructions():
    ...
    print("* If you are using the Encryption Solver, you will be given a 'key'.\n"
          ...
          "* If you are using the Decryption Solver, you can copy and paste the encrypted"
          " message from the Encrypted Solver.\n", break_lines, "\n"
          "* If your decrypted message doesn't match your original message, make sure you"
...
def encryption():
    ...
    # Create a list of possible set shift options
    shift_options = [3, 5, 7]
...
def decryption():
    ...
    # Turn users positive input into a negative integer
...
# Creating lists to tell users how to pick what solver they would like to use
options = ['To Encrypt,', 'To Decrypt,', 'To Exit,', 'Your']
enter = ['enter (a)', 'enter (b)', 'enter (c)', 'Answer: ']
option_message = "\n".join("{} {}".format(x, y) for x, y in zip(options, enter))

while True:
    user_action = encrypt_decrypt(option_message, )
    if user_action == "c":
        exit()
